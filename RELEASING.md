...
`rekursiv-ai/madcatter` triggers `.github/workflows/publish-pypi.yml`
...
```bash
# Latest version
curl -s https://pypi.org/pypi/madcatter/json | jq -r '.info.version'

# All published versions
curl -s https://pypi.org/pypi/madcatter/json | jq -r '.releases | keys[]'

# Or via pip
pip index versions madcatter
```
...
- Trusted publishing is configured on the `pypi` GitHub environment in
  `rekursiv-ai/madcatter`; rotate via PyPI project settings, not here.
