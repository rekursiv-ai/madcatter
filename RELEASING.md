...
1. **(First release only) Register the trusted publisher on PyPI.**
   ...
   https://pypi.org/manage/project/madcatter/settings/publishing/ and add
   a publisher with these exact values:

   | Field | Value |
   |---|---|
   | Owner | `rekursiv-ai` |
   | Repository name | `madcatter` |
...
3. **Refresh the lockfile and validate locally** (same checks CI runs):
   ...
   uv lock
   uv build
   uv run python -c "import madcatter; print(madcatter.__file__)"
   ```

4. **Commit and merge to `main`.** Confirm the published version:
   ```bash
   gh api repos/rekursiv-ai/madcatter/contents/pyproject.toml --jq .content | base64 -d | grep '^version'
   ```

5. **Cut the release** (this triggers the PyPI publish):
   ```bash
   gh release create vX.Y.Z --repo rekursiv-ai/madcatter --title "vX.Y.Z" --generate-notes
   ```

6. **Watch the workflow:**
   ```bash
   gh run watch --repo rekursiv-ai/madcatter $(gh run list --repo rekursiv-ai/madcatter --workflow publish-pypi.yml --limit 1 --json databaseId --jq '.[0].databaseId')
   ```

7. **Confirm it's live:**
   ```bash
   curl -s https://pypi.org/pypi/madcatter/json | jq -r '.info.version'   # should print X.Y.Z
   ```
   Browser: https://pypi.org/project/madcatter/

## If the workflow fails

- **`invalid-publisher`** ("valid token, but no corresponding
  publisher") — the trusted publisher isn't registered or doesn't match.
  Do step 1, then re-run: `gh workflow run publish-pypi.yml --repo rekursiv-ai/madcatter`.

- **Transient PyPI failure** — re-run without a new tag:
  `gh workflow run publish-pypi.yml --repo rekursiv-ai/madcatter`.
...
- **Must ship now, publishing still broken** — upload directly with the
  ...
  personal token can't publish `madcatter`):
  ```bash
  uv build --out-dir dist
  uv run --with twine twine check dist/madcatter-X.Y.Z*
  uv run --with twine twine upload dist/madcatter-X.Y.Z* -u __token__ -p "$PYPI_TOKEN_WORK"
  ```
